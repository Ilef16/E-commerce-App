...
[ApiController]
[Route("api/orders")]
public class CommandesController(ICommandeService commandeService)
    : ControllerBase
{
    [HttpGet]
    [ProducesResponseType(
        typeof(PagedResult<CommandeDto>),
        ...
        CancellationToken ct = default)
    {
        ...
        return Ok(
            await commandeService.GetPagedAsync(
                page,
                pageSize,
                ct));
    }
    ...
    {
        var commande =
            await commandeService.GetByIdAsync(id, ct);
...
public async Task<IActionResult> Create(
    [FromBody] CommandeWriteDto input,
    CancellationToken ct)
{
    try
    {
        ...
        CancellationToken ct)
    {
        return await Execute(
            () => commandeService.UpdateAsync(id, input, ct));
    }
        ...
        CancellationToken ct)
    {
        try
        {
            return await commandeService.DeleteAsync(id, ct)
                ? NoContent()
                : NotFound();
        }
        catch (InvalidOperationException ex)
    ...
    {
        return await Execute(
            () => commandeService.ValidateAsync(id, ct),
    ...
    {
        return await Execute(
            () => commandeService.CancelAsync(id, ct),
